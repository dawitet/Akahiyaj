...
class SecurityService @Inject constructor(
    ...
    fun validateGroupName(name: String): Result<String> {
        return when {
            name.isBlank() -> failure(AppError.ValidationError.InvalidInput("Group name cannot be empty").message)
            name.length < 3 -> failure(AppError.ValidationError.InvalidInput("Group name must be at least 3 characters").message)
            name.length > 50 -> failure(AppError.ValidationError.InvalidInput("Group name cannot exceed 50 characters").message)
            !name.matches(Regex("^[a-zA-Z0-9\\s\\u1200-\\u137F\\u1380-\\u139F\\u2D80-\\u2DDF]+$")) -> 
                failure(AppError.ValidationError.InvalidInput("Group name contains invalid characters").message)
            containsProfanity(name) -> failure(AppError.ValidationError.InvalidInput("Group name contains inappropriate content").message)
            else -> success(sanitizeInput(name))
        }
    }
    
    fun validateDestination(destination: String): Result<String> {
        return when {
            destination.isBlank() -> failure(AppError.ValidationError.InvalidInput("Destination cannot be empty").message)
            destination.length < 3 -> failure(AppError.ValidationError.InvalidInput("Destination must be at least 3 characters").message)
            destination.length > 100 -> failure(AppError.ValidationError.InvalidInput("Destination cannot exceed 100 characters").message)
            !destination.matches(Regex("^[a-zA-Z0-9\\s\\u1200-\\u137F\\u1380-\\u139F\\u2D80-\\u2DDF.,'-]+$")) -> 
                failure(AppError.ValidationError.InvalidInput("Destination contains invalid characters").message)
            else -> success(sanitizeInput(destination))
        }
    }
    
    fun validateChatMessage(message: String): Result<String> {
        return when {
            message.isBlank() -> failure(AppError.ValidationError.InvalidInput("Message cannot be empty").message)
            message.length > 500 -> failure(AppError.ValidationError.InvalidInput("Message cannot exceed 500 characters").message)
            containsProfanity(message) -> failure(AppError.ValidationError.InvalidInput("Message contains inappropriate content").message)
            containsSpam(message) -> failure(AppError.ValidationError.InvalidInput("Message appears to be spam").message)
            else -> success(sanitizeInput(message))
        }
    }
    
    fun validateUserName(name: String): Result<String> {
        return when {
            name.isBlank() -> failure(AppError.ValidationError.InvalidInput("Name cannot be empty").message)
            name.length < 2 -> failure(AppError.ValidationError.InvalidInput("Name must be at least 2 characters").message)
            name.length > 30 -> failure(AppError.ValidationError.InvalidInput("Name cannot exceed 30 characters").message)
            !name.matches(Regex("^[a-zA-Z\\s\\u1200-\\u137F\\u1380-\\u139F\\u2D80-\\u2DDF]+$")) -> 
                failure(AppError.ValidationError.InvalidInput("Name contains invalid characters").message)
            containsProfanity(name) -> failure(AppError.ValidationError.InvalidInput("Name contains inappropriate content").message)
            else -> success(sanitizeInput(name))
        }
    }
    ...
    suspend fun checkRateLimit(userId: String, action: String, maxRequests: Int = 10, windowMs: Long = 60000): Result<Unit> {
        ...
        return if (rateLimitInfo == null) {
            ...
            com.dawitf.akahidegn.core.result.success(Unit)
        } else {
            ...
            if (currentTime - rateLimitInfo.windowStartTime > windowMs) {
                ...
                com.dawitf.akahidegn.core.result.failure(AppError.RateLimitError.TooManyRequests("Too many requests. Please try again later.").message)
            } else {
    ...
    suspend fun validateSession(userId: String): Result<Unit> {
        ...
        return if (currentTime - lastActivity > sessionTimeout) {
            ...
            failure(AppError.AuthenticationError.SessionExpired.message)
        } else {
