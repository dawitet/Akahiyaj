...
import com.dawitf.akahidegn.core.result.error
...
class UserProfileRepositoryImpl @Inject constructor(
        ...
        return try {
            ...
            if (snapshot.exists()) {
                ...
                if (profile != null) {
                    ...
                    error(AppError.ValidationError.NotFound("Profile data is invalid").message)
                }
            } else {
                error(AppError.ValidationError.NotFound("User profile not found").message)
            }
        } catch (e: Exception) {
            error(AppError.NetworkError.RequestFailed(e.message ?: "Failed to get user profile").message)
        }
    }
    ...
    override suspend fun updateUserProfile(userProfile: UserProfile): Result<UserProfile> {
        return try {
            ...
            error(AppError.NetworkError.RequestFailed(e.message ?: "Failed to update profile").message)
        }
    }
    ...
    override suspend fun uploadProfileImage(imageUri: String): Result<String> {
        return try {
            val currentUser = auth.currentUser
                ?: return error(AppError.AuthenticationError.NotAuthenticated.message ?: "User not authenticated")
            ...
            error(AppError.UnknownError(e.message ?: "Failed to upload profile image").message)
        }
    }
    ...
    override suspend fun getUserAchievements(userId: String): Result<List<UserAchievement>> {
        return try {
            ...
            error(AppError.UnknownError(e.message ?: "Failed to get user achievements"))
        }
    }
    ...
    override suspend fun unlockAchievement(achievementId: String): Result<UserAchievement> {
        return try {
            val currentUser = auth.currentUser
                ?: return error(AppError.AuthenticationError.NotAuthenticated.message)
            ...
            error(AppError.UnknownError(e.message ?: "Failed to unlock achievement"))
        }
    }
    */
    ...
    override suspend fun getUserReviews(userId: String): Result<List<UserReview>> {
        return try {
            ...
            error(AppError.NetworkError.RequestFailed(e.message ?: "Failed to load reviews"))
        }
    }
    ...
    override suspend fun submitReview(review: UserReview): Result<UserReview> {
        return try {
            ...
            success(reviewWithId)
        } catch (e: Exception) {
            error(AppError.UnknownError(e.message ?: "Failed to submit review").message)
        }
    }
    ...
    override suspend fun getUserTripHistory(userId: String): Result<List<TripHistoryItem>> {
        return try {
            ...
            error(AppError.UnknownError(e.message ?: "Failed to get trip history").message)
        }
    }
    ...
    override suspend fun getUserAnalytics(userId: String): Result<UserAnalytics> {
        return try {
            ...
            error(AppError.UnknownError(e.message ?: "Failed to get user analytics").message)
        }
    }
